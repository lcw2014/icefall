...
class CLM(EncoderInterface):
    ...
    def __init__(
        ...
        # self.encoder_embed converts the input of shape (N, T, Vocab)
        ...
        self.embedding_pos = RelPositionalEncoding(d_model, dropout)

        self.encoder_layers = num_encoder_layers
        self.d_model = d_model # params.encoder_dim
        self.cnn_module_kernel = cnn_module_kernel
        self.causal = causal
        self.eos_id = eos_id
        self.sos_id = sos_id

        encoder_layer = CLMEncoderLayer(
            d_model=d_model,
            nhead=nhead,
        ...
        self.encoder_pos2 = RelPositionalEncoding(d_model, dropout)
        self.fc_lm = nn.Sequential(
            nn.Linear(output_dim, d_model),
            nn.LayerNorm(d_model),
        )
        MCM_layer = CLMEncoderLayer(
            d_model=d_model,
            nhead=nhead,
            dim_feedforward=dim_feedforward,
            dropout=dropout,
    ...
    def forward(
        self,
        x: torch.Tensor, # am output
        x_lens: torch.Tensor,
        lm_embeddings: torch.Tensor,
        y: torch.Tensor, # lm output
        y_lens: torch.Tensor,
        warmup: float = 1.0
        ...
        lm_embeddings = self.fc0(lm_embeddings) # (N, L+1, embedding_dim) -> (N, L+1, d_model)
        batch, length = lm_embeddings.shape[:2]
        input_pos = self.embedding_pos(lm_embeddings)[1][:,:length,:].repeat(batch,1,1)

        # print(lm_embeddings[0].shape,lm_embeddings[1].shape) #torch.Size([21, 101, 384]) torch.Size([1, 201, 384])
        input_pos = input_pos.permute(1, 0, 2)  # (N, L, d_model) -> (L, N, d_model)
        x, pos_emb1 = self.encoder_pos(x)
        x = x.permute(1, 0, 2)
        y = y.permute(1, 0, 2)
        # print(x.shape,y.shape,lm_embeddings.shape)


        # lengths = (((x_lens - 1) >> 1) - 1) >> 1
        y_lengths = y_lens
        assert input_pos.size(0) == y_lengths.max().item()
        src_key_padding_mask1 = make_pad_mask(y_lengths)

        x_lengths = x_lens
        # print(x.shape, x_lengths.max().item())
        assert x.size(0) == x_lengths.max().item()
        src_key_padding_mask2 = make_pad_mask(x_lengths)

        x = self.encoder(
            x, # (T', N, d_model)
            input_pos, # (L, N, d_model)
            pos_emb1,
            src_key_padding_mask=src_key_padding_mask2,
            trg_query_padding_mask=src_key_padding_mask1,
            warmup=warmup,
        )  # (L, N, d_model)

        x = self.fc1(x) # (L, N, d_model)
        x, pos_emb2 = self.encoder_pos2(x.permute(1, 0, 2))
        x = x.permute(1, 0, 2)

        # mask_prob = 0.2
        # y_mask = (torch.rand(y.shape[1]) < mask_prob).reshape(1,-1,1).repeat(y.shape[0],1,y.shape[-1])
        # y[y_mask] = 0
        ...
        y = self.MCM(
            ...
            y,
            pos_emb=pos_emb2,
            src_key_padding_mask=src_key_padding_mask1,
            trg_query_padding_mask=src_key_padding_mask1,
            warmup=warmup,
        )
        y = self.fc_out(y) # (L+1, N, V)
        y = y.permute(1, 0, 2)  # (N, L+1, V) ->(L+1, T, V)
...
class CLMEncoderLayer(nn.Module):
    """
    ...
    def __init__(
        ...
        super(CLMEncoderLayer, self).__init__()
    ...
    def forward(
        ...
        trg_query_padding_mask: Optional[Tensor] = None,
        warmup: float = 1.0,
    ) -> Tensor:
        """
        ...
        trg = trg + self.dropout(trg_att)

        # convolution module
        conv, _ = self.conv_module(trg, src_key_padding_mask=trg_query_padding_mask)
        trg = trg + self.dropout(conv)

        # feed forward module
        trg = trg + self.dropout(self.feed_forward(trg))

        trg = self.norm_final(self.balancer(trg))

        if alpha != 1.0:
            trg = alpha * trg + (1 - alpha) * trg_orig
    ...
    def forward(
        ...
        for i, mod in enumerate(self.layers):
            output = mod(
                ...
                trg_query_padding_mask=trg_query_padding_mask,
                warmup=warmup,
            )
...
class RelPositionMultiheadAttention(nn.Module):
    ...
    def multi_head_attention_forward(
        ...
        # print('qkv pos_emb: ',q.shape,k.shape,v.shape,pos_emb.shape,'\n')
        ...
        # (batch, 2*time2 -1, head, d_k) --> (batch, head, d_k, 2*time2 -1)
        p = p.permute(0, 2, 3, 1)
        ...
        matrix_bd = torch.matmul(q_with_bias_v, p)  # (batch, head, time1, 2*time2-1)
        # print(matrix_ac.shape,matrix_bd.shape)
        ...
        if key_padding_mask is not None:
            ...
            attn_output_weights = attn_output_weights.masked_fill(
                key_padding_mask.unsqueeze(1).unsqueeze(2), # (batch, 1, 1, time2)
                float("-inf"),
            )
            attn_output_weights = attn_output_weights.view(
                bsz * num_heads, tgt_len, src_len
