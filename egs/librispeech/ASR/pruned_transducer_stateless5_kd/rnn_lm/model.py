...
class RnnLmModel(torch.nn.Module):
    ...
    def get_logits_and_embeddings(self, tokens, sos_id, eos_id, blank_id):
        device = next(self.parameters()).device
    ...
    def get_logits_and_embeddings_and_loss(self, tokens, sos_id, eos_id, blank_id):
        device = next(self.parameters()).device
        ...
        # sys.exit()
        nll_loss = F.cross_entropy(
            logits.reshape(-1, self.vocab_size), y_tokens.reshape(-1), reduction="none"
        )
        mask = make_pad_mask(sentence_lengths).reshape(-1)
        nll_loss.masked_fill_(mask, 0)

        nll_loss = nll_loss.reshape(batch_size, -1)

        return logits, embedding, nll_loss
