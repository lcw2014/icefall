...
import torch
from model import CLTransducer
...
def beam_search(
    ...
    espnet/nets/beam_search_CLTransducer.py#L247 is used as a reference.
...
def modified_beam_search_lm_shallow_fusion(
    ...
    for (t, batch_size) in enumerate(batch_size_list):
        ...
        # print(decoder_input.shape) # [30,2]
        ...
        for i in range(batch_size):
            topk_log_probs, topk_indexes = ragged_log_probs[i].topk(beam)
            # print(topk_indexes) # [0,63,11,90]
            # print(topk_indexes.shape) # 4

            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                topk_hyp_indexes = (topk_indexes // vocab_size).tolist()
                topk_token_indexes = (topk_indexes % vocab_size).tolist()
                # print(topk_hyp_indexes) # [0,0,0,0]
                # print(topk_token_indexes) # [0,63,11,90]
            for k in range(len(topk_hyp_indexes)):
                hyp_idx = topk_hyp_indexes[k]
                hyp = A[i][hyp_idx]
        ...
        if len(token_list) != 0:
            ...
            if LM.lm_type == "rnn":
                ...
                # print(tokens_to_score.shape) # [90,1]
                hs = torch.cat(hs, dim=1).to(device)
                cs = torch.cat(cs, dim=1).to(device)
                state = (hs, cs)
            else:
        ...
        for i in range(batch_size):
            topk_log_probs, topk_indexes = ragged_log_probs[i].topk(beam)
            # print(topk_log_probs) # tensor([ -0.6615,  -0.7272,  -7.6458, -10.1969], device='cuda:0')
            # print(topk_log_probs.shape) # 4
            # sys.exit()
    ...
    for i in range(N):
        ...
        # print(sorted_ans[i])
        # print(ans[i])
    #     print(len(sorted_ans[i]),len(ans[i]))
...
def modified_beam_search_lm_clm_shallow_fusion(
    model: CLTransducer,
    encoder_out: torch.Tensor,
    encoder_out_lens: torch.Tensor,
    sp: spm.SentencePieceProcessor,
    ...
    assert LM is not None
    lm_scale = LM.lm_scale
    encoder_out_origin = encoder_out.clone()
    # print(encoder_out.shape) # torch.Size([30, 332, 384])
    # sys.exit()
    ...
    for b in B:
      ...
      temp2 = []
      temp3 = []
      for key in b._data.keys():
        temp1.append(torch.tensor(b._data[key].ys[1:]).to(torch.int64).to(device))
        temp2.append(len(b._data[key].ys[1:]))
        temp3.append(b._data[key].log_prob.clone().detach().to(device))
      hyps_ks.append(len(b._data.keys()))
      hypsB.append(temp1)
      ylenss.append(torch.tensor(temp2).to(torch.int64).to(device))
      log_probss.append(temp3)
      # print(len(b._data.keys()))
    best_hyps = []
    for i, hyps4 in enumerate(hypsB):
      y = pad_sequence(hyps4,batch_first=True,padding_value=blank_id)
      lm_embedding = LM.lm.input_embedding(y)
      rnn_out, _ = LM.lm.rnn(lm_embedding)
      lm_outputs = LM.lm.output_linear(rnn_out)

      # print(encoder_out_origin[i,:encoder_out_lens[i]].repeat(hyps_ks[i],1,1).shape)
      # print(encoder_out_lens[i].repeat(hyps_ks[i]))
      clm_out = model.clm(
        x=encoder_out_origin[i,:encoder_out_lens[i]].repeat(hyps_ks[i],1,1),
        x_lens=encoder_out_lens[i].repeat(hyps_ks[i]),
        lm_embeddings=lm_embedding,
        y=lm_outputs,
        y_lens=ylenss[i])
      best = torch.full((hyps_ks[i],), -float('inf')).to(device)
      for l in range(hyps_ks[i]):
        tmp = clm_out[l]
        tmp_len = tmp.max(dim=1)[1]
        tmp_len = len(tmp_len[tmp_len != 1])
        if tmp_len == 0:
          print('len 0')
          continue
        tmp = clm_out[l][:,:tmp_len].max(dim=1)[0].sum() / tmp_len
        best[l] = tmp

        best[l] += log_probss[i][l]
      # print(best)
      # print(log_probss[i])
      best_idx = torch.argmax(best)
      ...
      # best_hyp = clm_out[best_idx].max(dim=1)[1]
      # best_hyp = best_hyp[best_hyp != 1][best_hyp != 0].tolist()
      best_hyp = hyps4[best_idx][hyps4[best_idx] != 1].tolist()
    ...
    ans = []
    for i in range(N):
      ans.append(best_hyps[unsorted_indices[i]])
