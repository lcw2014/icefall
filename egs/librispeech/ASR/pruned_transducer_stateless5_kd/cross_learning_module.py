...
class CLM(EncoderInterface):
    ...
    def forward(
        ...
        lm_embeddings = self.fc0(lm_embeddings) # (N, L+1, embedding_dim) -> (N, L+1, D)
        lm_embeddings, pos_emb1 = self.encoder_pos(lm_embeddings)
        lm_embeddings = lm_embeddings.permute(1, 0, 2)  # (N, L, D) -> (L, N, D)
        x = x.permute(1, 0, 2)
        y = y.permute(1, 0, 2)
        ...
        x = self.encoder(
            lm_embeddings, # (L, N, d_model)
            x, # (T`, N, d_model)
            pos_emb1,
            src_key_padding_mask=src_key_padding_mask1,
            trg_query_padding_mask=src_key_padding_mask2,
            warmup=warmup,
        )  # (T, N, d_model)

        x = self.fc1(x) # (T, N, d_model)
