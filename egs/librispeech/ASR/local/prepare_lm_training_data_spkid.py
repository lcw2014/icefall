...
def main():
    ...
    processed = 0
    if len((args.lm_data.split())) > 1:
        for i in args.lm_data.split():
            with open(f'{args.out_dir}/userlibri-lm-norm-{i}.txt') as f:
                while True:
                    line = f.readline()
                    if line == "":
                        break
        ...
        with open(f'{args.out_dir}/userlibri-lm-norm-{args.lm_data}.txt') as f:
            while True:
                line = f.readline()
                if line == "":
                    break
